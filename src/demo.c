...
/*
	...
	start_fetch = get_time_in_ms();
        ...
        if (letter_box){
            ...
            printf("letter boxxxxxxxxxxxxxxxxxxxxxxxx");
            }
        else
...
void demo(char *datacfg, char *cfgfile, char *weightfile, float thresh, float hier_thresh, int cam_index, const char *filename, int frame_skip, char *prefix, char *out_filename, int mjpeg_port, int json_port, int dont_show, int ext_output, int letter_box_in, int time_limit_sec, char *http_post_host, int benchmark, int benchmark_layers, int w, int h, int cam_fps)
{
    ...
    fprintf(stderr, "ERROR: Set ERO_SLACK in Makefile\n");
    exit(0);
#elif (defined CONTENTION_FREE) 
    fprintf(stderr, "ERROR: Set CONTENTION_FREE in Makefile\n");
    exit(0);
#elif (defined NEW_ZERO_SLACK)
    fprintf(stderr, "ERROR: Set NEW_ZERO_SLACK in Makefile\n");
    exit(0);
#endif    
...
*/
    ...
    fetch_in_thread(0); //fetch_in_thread_sync(0); 
    det_img = in_img;
    det_s = in_s;

    fetch_in_thread(0); //fetch_in_thread_sync(0);
    detect_in_thread(0); //fetch_in_thread_sync(0);
    det_img = in_img;
    det_s = in_s;
    ...
    for (j = 0; j < avg_frames / 2; ++j) {
        ...
        fetch_in_thread(0); //fetch_in_thread_sync(0);
        detect_in_thread(0); //detect_in_thread_sync(0); 
        det_img = in_img;
        det_s = in_s;
    }
    ...
    while(1){
	    ...
	    start_loop[buff_index] = get_time_in_ms();
            ...
            if (!benchmark) if (pthread_create(&fetch_thread, 0, fetch_in_thread, 0)) error("Thread creation failed", DARKNET_LOC);
            if (pthread_create(&detect_thread, 0, detect_in_thread, 0)) error("Thread creation failed", DARKNET_LOC);
